...
```{r echo = FALSE}
Anova_SigLRC <- readRDS(file = file.path("..", "Output", "TablesRds", "BalticO2_Anova_TauLRC.Rds"))
Anova_SigLRC<-Anova_SigLRC %>% 
  rename(p_value = "Pr(>F)") %>%
  rename(F_value = "F value") %>%
  mutate(Df=as.character(Df))
...
```{r echo = FALSE}
Anova_SigLRC <- readRDS(file = file.path("..", "Output", "TablesRds", "BalticO2_Anova_qpLRC.Rds"))
Anova_SigLRC<-Anova_SigLRC %>% 
  rename(p_value = "Pr(>F)") %>%
  rename(F_value = "F value") %>%
  mutate(Df=as.character(Df))
