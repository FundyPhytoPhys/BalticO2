...
PSII_cap<- glue("PSII to Chl *a* ratio vs. growth waveband (nm). ETR (µmol e^−^ PSII^−1^ s^−1^) and *JV*~PSII~ (µmol e^−^ Chl *a*^−1^ s^−1^) were estimated using FRRf induction curves with excitation (columns) at Ex~445nm~, blue for PC-rich (green circle) or PE-rich (red circle) cultures of *Synechococcus* sp. grown at spectral bandwidths of 450, 470, 530, or 620 nm and O~2~ concentrations of 250 µM (open symbols and dashed line) or 2.5 µM (closed symbols and solid line). Data not presented for those PE-rich cultures which showed negligible growth under 405, 450, 730 nm and 250 µM O~2~; nor for those PC-rich cultures which showed negligible growth under 405 nm and 250 µM O~2~.", .sep = " ")
...
```{r echo = FALSE}
Anova_qpLRC <- readRDS(file = file.path("..", "Output", "TablesRds", "BalticO2_Anova_JVPSII_eLsLRC.Rds"))
Anova_qpLRC<-Anova_qpLRC %>% 
  rename(p_value = "Pr(>F)") %>%
  rename(F_value = "F value") %>%
  mutate(Df=as.character(Df))
