...
We used R version 4.3.0 [@rcore] running under RStudio [@posit]. We performed three-way factorial ANOVA (*aov()* function; R Base package) to determine whether strain, growth waveband, O~2~ concentration, and their interactions, significantly influence the chlorophyll-specific exponential growth rate (µ; d^−1^; Tab. S4) and pigment content (Tab. S5). We also performed three-way factorial ANOVA (*aov()* function) to determine whether strain, Actinic PAR, O~2~ concentration, and their interactions, significantly influence the σ~PSII~ (Tab. S6); τ~PSII~ (Tab. S7); or q~P~ (Tab. S8).

We used *t*-tests of linear fits to compare data across different strains and O~2~ concentration for a given growth waveband, for chlorophyll-specific exponential growth rate vs. Phycobiliproteins to Chl *a* ratio (Tab. S9). We also performed *t*-tests of linear fits to compare data across different strains and O~2~ concentration in situations in which cultures were excited by, and growing in, corresponding growth wavebands of 450, 470, 530, or 620 nm, for chlorophyll-specific exponential growth rate vs. *JV*~PSII~ (Tab. S10). Statistical differences for all analyses were determined at significance level α = 0.05. 
