...
JazDataRoundMeta2 %>%
  ...
  stat_wl_strip(aes(x = nm), ymin = -Inf, ymax = -0.1, alpha = 0.5) +
  scale_fill_identity() +
  coord_flip()+
...
JazDataRoundMeta2 %>%
  ...
  # coord_cartesian(ylim = c (-0.01, 1.6)) +
  #ggh4x::facet_nested(rows = vars(factor(O2)),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  ggh4x::facet_nested(cols = vars(factor(Oxygen, levels=c("250~µM","2.5~µM"))), labeller = label_parsed) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
        panel.spacing.x = unit(0.4, 'cm'),
...
```{r save plot}
#ggsave(file = file.path(FigPath, paste("Fig_GrowthRate2",".png",sep = "")), height=8, width= 8,  dpi = 300, limitsize = TRUE)
```
...
MCGrowthJVPSIIAllFitBA127 <- MultiCultiGrowth1 %>%
  filter(Strain == "PE-rich_127") %>% 
  nest(data = -c("Strain", "O2", "Oxygen")) %>%
  mutate(GRCplatt_Model = map(data, ~nlsLM(deltaOD_Lmu_corr ~ grcplatt(I = WLNum, a, b, Pmax), data = .x, start = list(a =0.1/400, b = 0.4/500, Pmax = 2), lower = c(0, 0, 0)))) %>%
...
GRCplatt_PredictGrowthJVPSIIAll127 <- MCGrowthJVPSIIAllFitBA127 %>%
  unnest(GRCplatt_Predict)%>%
  unnest(GRCplatt_Param)


MCGrowthJVPSIIAllFitBA77 <- MultiCultiGrowth1 %>%
  ...
  mutate(GRCplatt_Param = map(GRCplatt_Model, tidy),
         GRCplatt_Predict = map(GRCplatt_Model, possibly(augment)),
         GRCplatt_Glance = map(GRCplatt_Model, possibly(glance)))
GRCplatt_PredictGrowthJVPSIIAll77 <- MCGrowthJVPSIIAllFitBA77 %>%
  unnest(GRCplatt_Predict)%>%
  unnest(GRCplatt_Param)
...
#   geom_errorbar(aes(x = WLNum, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se, colour = as.factor(Strain)), width=0, size=0.3, show.legend = F, MultiCultiGrowth1) +
#   
#   geom_line(aes(WLNum, .fitted), colour = "palegreen3", size = 0.3, linetype = "solid", show.legend = F, GRCplatt_PredictGrowthJVPSIIAll77) +
#   geom_line(aes(WLNum, .fitted), colour = "brown4", size = 0.3, linetype = "solid", show.legend = F, GRCplatt_PredictGrowthJVPSIIAll127) +
#      
#   scale_colour_manual(values = c("palegreen3", "brown4"), name="", labels = lab1) +
#   scale_shape_manual(values = c(16, 18), name="", labels = lab1) +
#   scale_linetype_manual(values = c("solid", "dashed"), name="", labels = lab1) +
...
#   scale_fill_identity() +
#   labs(y = "µ " ~ "("~d^-1~") and normalized emission spectra", x = "Growth wavelength (nm)") +
#   ggh4x::facet_nested(rows = vars(Oxygen), labeller = label_parsed) +
#   theme_bw() +
#   theme(panel.grid.minor = element_blank(),
#         panel.grid.major = element_blank(),
#         panel.spacing.x = unit(0.4, 'cm'),
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_GrowthRate2",".png",sep = "")), height=6, width= 7,  dpi = 300, limitsize = TRUE)
```
