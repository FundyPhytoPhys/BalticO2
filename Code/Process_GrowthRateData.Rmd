...
MultiCultiGrowth %>%
  ...
  filter(WL != "WW") %>%
  ggplot() +
  geom_point(aes(x = WLNum, y = deltaOD_Lmu_corr, colour = as.factor(Strain)), size = 3, show.legend = T) +
  geom_line(aes(x = WLNum, y = deltaOD_Lmu_corr, colour = as.factor(Strain)), show.legend = F) +
  ...
  scale_fill_identity() +
  scale_colour_manual(values = c("palegreen3", "brown4"), name="") +
  labs(y = "Chlorophyll-specific exponential growth rate " ~ "("~d^-1~")", x = "Wavelength (nm)") +
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_GrowthRate",".png",sep = "")), height=4, width= 6,  dpi = 300, limitsize = TRUE)
```
