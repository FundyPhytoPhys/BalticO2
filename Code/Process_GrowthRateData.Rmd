...
MultiCultiGrowth %>%
  filter(WL != "WW") %>% 
  ggplot() +
  geom_point(aes(x = WL, y = deltaOD_Lmu_corr,colour=as.factor(O2)), size = 3.5, alpha = 0.9, show.legend = T) +
...
MCGrowthRate %>%
  filter(OD720_Lmu_se<0.2) %>% 
  filter(OD720_Lmu_corr>0) %>% 
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") %>%
  ggplot() +
  geom_point(aes(x = meanPUR, y = OD720_Lmu_corr, colour = as.factor(WL)), alpha = 0.4, size = 4, show.legend = T) +
  geom_line(aes(x = meanPUR, y = OD720_Lmu_corr, linetype = as.factor(O2)), show.legend = T) +
  geom_errorbar(aes(x = meanPUR, ymin = OD720_Lmu_corr - OD720_Lmu_se, ymax = OD720_Lmu_corr + OD720_Lmu_se, colour = as.factor(WL)), width=0, size=0.3, show.legend = F) +
...
GrowthPURPlot<-MCGrowthRate %>%
  ...
  ggplot() +
  geom_point(aes(x = meanPUR, y = deltaOD_Lmu_corr, colour = as.factor(WL)), alpha = 0.4, size = 4, show.legend = T) +
  geom_smooth(aes(x = meanPUR, y = deltaOD_Lmu_corr, linetype = as.factor(O2)), show.legend = T, method = "lm") +
  geom_errorbar(aes(x = meanPUR, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se, colour = as.factor(WL)), width=0, size=0.3, show.legend = F) +
    geom_text(data=data_textD, aes(x=150, y=0.2, label=label), size=6, parse = TRUE) +
  #stat_wl_strip(aes(x = meanPUR), ymin = -Inf, ymax = -0.08, alpha = 0.5) +
  scale_fill_identity() +
  #scale_colour_manual(values = c("royalblue4", "steelblue3", "lightseagreen", "limegreen", "coral","brown1","indianred4"), name="", labels = lab3) +
...
JazDataRoundMeta2 %>%
  ...
  geom_point(aes(x = WLNum, y = OD720_Lmu_corr, shape = as.factor(O2)), size = 4, show.legend = T, MultiCultiGrowth1) +
  
    geom_point(aes(x = WLNum, y = OD720_Lmu_corr), colour = "palegreen3", shape = 1, size = 4, show.legend = F, MultiCultiGrowth1PC21) +
    geom_point(aes(x = WLNum, y = OD720_Lmu_corr), colour = "brown4", shape = 1, size = 4, show.legend = F, MultiCultiGrowth1PE21) +
    geom_point(aes(x = WLNum, y = OD720_Lmu_corr), colour = "palegreen3", shape = 16, size = 4, show.legend = F, MultiCultiGrowth1PC0) +
    geom_point(aes(x = WLNum, y = OD720_Lmu_corr), colour = "brown4", shape = 16, size = 4, show.legend = F, MultiCultiGrowth1PE0) +
  
  geom_line(aes(x = WLNum, y = OD720_Lmu_corr, linetype = as.factor(O2)), show.legend = T, MultiCultiGrowth1) +

  geom_errorbar(aes(x = WLNum, ymin = OD720_Lmu_corr - OD720_Lmu_se, ymax = OD720_Lmu_corr + OD720_Lmu_se, colour = as.factor(Strain)), width=0, size=0.3, show.legend = F, MultiCultiGrowth1) +
    #geom_line(aes(x = WLNum, y = deltaOD_Lmu_corr, linetype = as.factor(O2)), size = 0.6, show.legend = T) +
  #geom_line(aes(x = WLNum, y = svr_pred, linetype = as.factor(O2)), size = 0.6, show.legend = T, MCDataHourSVR) +
  ...
  stat_wl_strip(aes(x = nm), ymin = -Inf, ymax = -0.06, alpha = 0.5) +
  scale_fill_identity() +
  labs(y = "µ " ~ "("~d^-1~") and normalized emission spectra", x = "Growth wavelength (nm)") +
  ggh4x::facet_nested(rows = vars(Strain), labeller = label_parsed) +
  theme_bw() +
...
FigGrowthPig<-MCGrowthPig %>%
  filter(deltaOD_Lmu_corr>0) %>% 
  ggplot() +
  geom_point(aes(x = meanPhycoChlaRatio, y = deltaOD_Lmu_corr, colour = as.factor(Strain), shape = as.factor(O2)), size = 4, show.legend = T) +
  geom_smooth(aes(x = meanPhycoChlaRatio, y = deltaOD_Lmu_corr, linetype = as.factor(O2)), show.legend = T, alpha = 0.2, method = "lm") +
  ...
  geom_errorbar(aes(x = meanPhycoChlaRatio, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se, colour = as.factor(Strain)), width=0, size=0.3, show.legend = F) +
    #geom_line(aes(x = WLNum, y = deltaOD_Lmu_corr, linetype = as.factor(O2)), size = 0.6, show.legend = T) +
  #geom_line(aes(x = WLNum, y = svr_pred, linetype = as.factor(O2)), size = 0.6, show.legend = T, MCDataHourSVR) +
  geom_text(data=data_textD, aes(x=6, y=0.2, label=label), size=6, parse = TRUE) +
