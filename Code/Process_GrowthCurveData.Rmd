...
data_texta <- data.frame(WL = c("730"), Strain  = c("PC-rich"), label = c('italic(a)'))
...
OD680<-MultiCultiDataAll %>%
  ...
  geom_line(aes(x = Time_h/24, y = meanOD680_h, colour = as.factor(O2)), size = 0.4, show.legend = T) +
  geom_text(data=data_texta, aes(x=7, y=0.7, label=label), size=6, parse = TRUE) +
  #scale_colour_discrete(type=c("palegreen3", "brown4")) +
  scale_colour_discrete(type=c("black", "deepskyblue2"), name="", labels = lab2) +
...
fig.height = 8, fig.width = 8, warning = FALSE
```{r create GrowthCurve plot}
data_textb <- data.frame(WL = c("730"), Strain  = c("PC-rich"), label = c('italic(b)'))
...
OD720<-MultiCultiDataAll %>%
  ...
  geom_line(aes(x = Time_h/24, y = meanOD720_h, colour = as.factor(O2)), size = 0.4, show.legend = F) +
  geom_text(data=data_textb, aes(x=7, y=0.5, label=label), size=6, parse = TRUE) +
  scale_colour_discrete(type=c("black", "deepskyblue2"), name="", labels = lab2) +
  scale_linetype_manual(values = c("solid", "dashed"), name="", labels = lab2) +
...
fig.height = 8, fig.width = 8, warning = FALSE
```{r create GrowthCurve plot}
data_textc <- data.frame(WL = c("730"), Strain  = c("PC-rich"), label = c('italic(c)'))
...
ODDelta<-MultiCultiDataAll %>%
  ...
  #geom_area(aes(x = Time_h, y = meanActinic_par_h), size = 0.1, fill = "tan1", alpha = 0.6) +
  geom_line(aes(x = Time_h/24, y = meanDeltaOD_h, colour = as.factor(O2)), size = 0.4, show.legend = F) +
  
  geom_line(aes(x = time_h/24, y = meanfittedDeltaOD_h, colour = as.factor(O2)), size = 0.8, alpha = 0.9, show.legend = F, MCGrowthRateFitData) +
  
  geom_text(data=data_textc, aes(x=7, y=0.15, label=label), size=6, parse = TRUE) +
  scale_colour_discrete(type=c("black", "deepskyblue2"), name="", labels = lab2) +
  scale_linetype_manual(values = c("solid", "dashed"), name="", labels = lab2) +
  labs(y = "Optical density (ΔOD)", x = "Elapsed time (d)") +
  #ggh4x::facet_nested(cols = vars(WL), rows = vars(factor(O2, levels=c("21","0"))), labeller = label_parsed) +
    ggh4x::facet_nested(cols = vars(WL), rows = vars(Strain), labeller = label_parsed) +
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("SFig_GrowthCurve2",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
...
```{r read file}
MultiCultiGrowthFile <- "../Data/CleanedData/CleanedMCData/PICO_Cleaned_MCDataFit.Rds"
...
MCGrowthRateFitData %>%
  ...
  geom_line(aes(x = time_h, y = meanfittedDeltaOD_h, colour = as.factor(O2)), size = 0.7, alpha = 0.9, show.legend = T) +
  #scale_colour_discrete(type=c("palegreen3", "brown4"), name="") +
  # scale_y_continuous(breaks=seq(0, 1, by = 0.25)) +
  # coord_cartesian(ylim = c(0, 1.0)) +
  labs(y = "Optical density (\u0394 OD)", x = "Elapsed time (h)") +
    ggh4x::facet_nested(cols = vars(WL), rows = vars(factor(Strain)), labeller = label_parsed) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
        axis.text = element_text(size=12),
