...
MultiCultiFile <- "../Data/ImportedData/ImportedMCData/20220329_PICO_MCMIX006_RUN57_TargetDataMetaFilter.Rds"
...
MultiCultiData57 <- MultiCultiData %>%   
...
```{r Import Run and calculate mean OD, echo = FALSE, warning = FALSE}
MultiCultiFile <- "../Data/ImportedData/ImportedMCData/20220407_PICO_MCMIX006_RUN61_TargetDataMetaFilter.Rds"
MultiCultiFileName <- str_split(string = MultiCultiFile, "/")[[1]][3] %>%
  str_remove(pattern = ".Rds") 
MultiCultiData <- readRDS(MultiCultiFile)  
...
MultiCultiData61 <- MultiCultiData %>%   
  group_by(SampleID, Day, ToD, time_h) %>% 
  summarize(Run, SampleID, Strain, ExpDate, Filename, Tube, time_h, ToD, Day, ExpDate, Actinic_par, OD680, OD720, DeltaOD, Par_ue, Photoperiod, O2, WL, LightShape, ExpEndDate, meanActinic_par_h = mean(Actinic_par), meanOD680_h = mean(OD680), meanOD720_h = mean(OD720), meanDeltaOD_h = mean(DeltaOD)) %>%
  ungroup() %>% 
  select(-c(Actinic_par, OD680, OD720, DeltaOD)) %>% 
...
```{r Import Run and calculate mean OD, echo = FALSE, warning = FALSE}
MultiCultiFile <- "../Data/ImportedData/ImportedMCData/20220609_PICO_MCMIX006_RUN76_TargetDataMetaFilter.Rds"
MultiCultiFileName <- str_split(string = MultiCultiFile, "/")[[1]][3] %>%
  str_remove(pattern = ".Rds") 
MultiCultiData <- readRDS(MultiCultiFile)  
...
MultiCultiData76 <- MultiCultiData %>%   
  group_by(SampleID, Day, ToD, time_h) %>% 
  summarize(Run, SampleID, Strain, ExpDate, Filename, Tube, time_h, ToD, Day, ExpDate, Actinic_par, OD680, OD720, DeltaOD, Par_ue, Photoperiod, O2, WL, LightShape, ExpEndDate, meanActinic_par_h = mean(Actinic_par), meanOD680_h = mean(OD680), meanOD720_h = mean(OD720), meanDeltaOD_h = mean(DeltaOD)) %>%
  ungroup() %>% 
  select(-c(Actinic_par, OD680, OD720, DeltaOD)) %>% 
...
```{r Import Run and calculate mean OD, echo = FALSE, warning = FALSE}
MultiCultiFile <- "../Data/ImportedData/ImportedMCData/20230816_PICO_MCMIX004_RUN122_TargetDataMetaFilter.Rds"
MultiCultiFileName <- str_split(string = MultiCultiFile, "/")[[1]][3] %>%
  str_remove(pattern = ".Rds") 
MultiCultiData <- readRDS(MultiCultiFile)  
...
MultiCultiData122 <- MultiCultiData %>%   
  group_by(SampleID, Day, ToD, time_h) %>% 
  summarize(Run, SampleID, Strain, ExpDate, Filename, Tube, time_h, ToD, Day, ExpDate, Actinic_par, OD680, OD720, DeltaOD, Par_ue, Photoperiod, O2, WL, LightShape, ExpEndDate, meanActinic_par_h = mean(Actinic_par), meanOD680_h = mean(OD680), meanOD720_h = mean(OD720), meanDeltaOD_h = mean(DeltaOD)) %>%
  ungroup() %>% 
  select(-c(Actinic_par, OD680, OD720, DeltaOD)) %>% 
...
```{r Import Run and calculate mean OD, echo = FALSE, warning = FALSE}
MultiCultiFile <- "../Data/ImportedData/ImportedMCData/20230830_PICO_MCMIX006_RUN124_TargetDataMetaFilter.Rds"
MultiCultiFileName <- str_split(string = MultiCultiFile, "/")[[1]][3] %>%
  str_remove(pattern = ".Rds") 
MultiCultiData <- readRDS(MultiCultiFile)  
...
MultiCultiData124 <- MultiCultiData %>%   
  group_by(SampleID, Day, ToD, time_h) %>% 
  summarize(Run, SampleID, Strain, ExpDate, Filename, Tube, time_h, ToD, Day, ExpDate, Actinic_par, OD680, OD720, DeltaOD, Par_ue, Photoperiod, O2, WL, LightShape, ExpEndDate, meanActinic_par_h = mean(Actinic_par), meanOD680_h = mean(OD680), meanOD720_h = mean(OD720), meanDeltaOD_h = mean(DeltaOD)) %>%
  ungroup() %>% 
  select(-c(Actinic_par, OD680, OD720, DeltaOD)) %>% 
...
MultiCultiDataAll<-rbind(MultiCultiData57, MultiCultiData61, MultiCultiData76, MultiCultiData122, MultiCultiData124)

rm(MultiCultiData57, MultiCultiData61, MultiCultiData76, MultiCultiData122, MultiCultiData12)
```
...
MultiCultiDataAll %>%
  ...
  geom_line(aes(x = Time_h, y = meanOD680_h, colour = as.factor(O2)), size = 0.5, show.legend = T) +
...
```{r save plot}
ggsave(file = file.path(PlotsPath, paste("GrowthCurve180_SupPlot",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
