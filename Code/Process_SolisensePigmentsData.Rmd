...
SolisenseFile <- "../Data/ImportedData/ImportedSolisenseData/BalticO2_Imported_SolisenseLightnew.Rds"  
...
Solisense1sFile <- "../Data/ImportedData/ImportedSolisenseData/BalticO2_Imported_SolisenseDarkafterLightnew.Rds"
...
# filter(Par_ue!=30) %>% 
  ggplot() +
  geom_point(aes(x = Par_ue, y = Sig, colour = as.factor(O2)), size = 3.5, show.legend = T) +
  facet_grid(rows = vars(Strain, Ex_WL), cols = vars(WL)) +
  theme_bw()
```
...
```{r filter outliers}
#Filter three unrevelant data point at 445nm
...
SolFits %>%
  ...
  filter(WL!= "WW") %>% 
  ggplot() +
  geom_point(aes(x = Ex_WL, y = Sig, colour = as.factor(O2)), size = 3.5, show.legend = T) +
  facet_grid(rows = vars(Strain), cols = vars(WL)) +
  theme_bw()
...
SolFits %>%
  ...
  filter(WL!= "WW") %>% 
  ggplot() +
  geom_point(aes(x = WL, y = Sig, colour = as.factor(O2)), size = 3.5, show.legend = T) +
...
SolFits %>%
  filter(Par_ue !=30) %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = Sig_nm2psii, colour = as.factor(Strain)),  alpha = 0.8, size = 3, show.legend = T) +
  scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
  ggh4x::facet_nested(rows = vars(Ex_WL), cols = vars(WL, O2), labeller = label_parsed) +
  theme_bw() 
...
```{r save plot}
# ggsave(file = file.path(PlotsPath, paste("Solisense4_SupPlot",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```




# Calculate Sigma mean from ~2 replica measured on the same day
# ~4 replica b/c 2 replica * 2 values ("up" and "down")

```{r calculate Sigma mean from 3 replica measured on the same day}
SolFits <- SolFits %>%
  group_by(SampleID, Ex_WL, ObsDate, Par_ue, Photoperiod, WL, O2) %>%
  
  summarize(SampleID, Ex_WL, FilenameSolisense, Run, Strain, ExpDate, Par_ue, Photoperiod, O2, WL,  PARPhotonDose_day, ObsDate, ObsTime, Sig, Sig_nm2psii, ActPARCorr_photonsm2s, Sigdark_nm2psii, FoOxbo, qpOxbo, JVPSII_aLHIIOxbomax, JVPSII_ETRtauav_FoSig, JVPSII_ETRqpOxbo_FoSig, JVPSII_ETRtauav_aLHII_Sig, JVPSII_ETRqpOxbo_aLHII_Sig, 
...
# Calculate Sigma1s mean from ~2 replica measured on the same day
# ~4 replica b/c 2 replica * 2 values ("up" and "down")
...
SolFits1s <- SolFits1s %>%
  ...
  summarize(SampleID, Ex_WL, FilenameSolisense, Run, Strain, ExpDate, Par_ue, Photoperiod, O2, WL,  PARPhotonDose_day, ObsDate, ObsTime, Sig1s, Sig1s_nm2psii, ActPARCorr_photonsm2s, 
...
Make "one big - All" mean to combine all days (only Exp) and all replica when samples were measured 
(This give me only one value of JVPSII for further growth vs JVPSII plot and sig plot as well)


# Calculate Sigma mean from all data measured on Exp phase (filter unrevelant data)
# Usually Allmean contains 12 or 30 replica - 2 or 5 different days of measurements * 3 replica * 2 values (up and down)
...
SolFitsExp<-SolFits %>% 
  ...
  summarize(SampleID, Ex_WL, FilenameSolisense, Run, Strain, ExpDate, Par_ue, Photoperiod, O2, WL,  PARPhotonDose_day, ObsDate, ObsTime, Sig, Sig_nm2psii, meanSig_nm2psii, sdSig_nm2psii, ActPARCorr_photonsm2s, Sigdark_nm2psii, meanSigdark_nm2psii, sdSigdark_nm2psii, FoOxbo, qpOxbo, JVPSII_aLHIIOxbomax, JVPSII_ETRtauav_FoSig, JVPSII_ETRqpOxbo_FoSig, JVPSII_ETRtauav_aLHII_Sig, JVPSII_ETRqpOxbo_aLHII_Sig, E_days, Time_h,
...
#   filter(Par_ue != 30) %>% 
#   ggplot() +
#   geom_point(aes(x = E_days, y = Sig_nm2psii, colour = as.factor(Strain)), size = 3.5, show.legend = T) +
...
#   scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
#   ggh4x::facet_nested(cols = vars(WL), rows = vars(Ex_WL, O2), labeller = label_parsed) +
#   theme_bw() 
# 
# SolFitsExp %>% 
#   filter(Par_ue != 30) %>% 
#   ggplot() +
#   geom_point(aes(x = E_days, y = meanSig_nm2psii, colour = as.factor(Strain)), size = 3.5, show.legend = T) +
#   # geom_errorbar(aes(x = PARPhotonDose_day, ymin = meanSig_nm2psii - sdSig_nm2psii, ymax = meanSig_nm2psii + sdSig_nm2psii, colour=as.factor(Par_ue))) +
#   scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
#   ggh4x::facet_nested(cols = vars(WL), rows = vars(Ex_WL, O2), labeller = label_parsed) +
#   theme_bw() 
