...
SolisenseFile <- "../Data/ProcessedData/ProcessedSolisenseData/BalticO2_Processed_SolisenseData.Rds"  
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_SigPigCorr",".png",sep = "")), height=5.5, width= 6,  dpi = 300, limitsize = TRUE)
```
...
SolFits445590<-rbind(SolFitsMetaLightCorr1, SolFitsMetaLightCorr2)
...
```{r save rds}
saveRDS(SolFitsPigmentExpChoose, file.path(DataOut, paste(Project, "Processed_SolisensePigmentsData.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
