...
SolFitsMetaLRCTrim %>%
  ...
  #geom_point(aes(x = ActPARCorr, y = Sig_nm2psii, colour = as.factor(Strain)), size = 1, alpha = 0.2, show.legend = F) +
    ...
    geom_point(aes(x = ActPARCorr, y = AllmeanJVPSII_umol_eLs), colour = "palegreen3", shape = 1, size = 3, show.legend = F, MultiCultiGrowth1PC21) +
    geom_point(aes(x = ActPARCorr, y = AllmeanJVPSII_umol_eLs), colour = "brown4", shape = 1, size = 3, show.legend = F, MultiCultiGrowth1PE21) +
    geom_point(aes(x = ActPARCorr, y = AllmeanJVPSII_umol_eLs), colour = "palegreen3", shape = 16, size = 3, show.legend = F, MultiCultiGrowth1PC0) +
    geom_point(aes(x = ActPARCorr, y = AllmeanJVPSII_umol_eLs), colour = "brown4", shape = 16, size = 3, show.legend = F, MultiCultiGrowth1PE0) +
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("SFig_CalibJVPSII_eLs",".png",sep = "")), height=8, width= 6,  dpi = 300, limitsize = TRUE)
```
...
UncalibJVPSII<-SolFitsMetaLRCTrimChoosen %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanJVPSII_umol_eLs, shape = as.factor(O2)), size = 3, show.legend = F) +
  ...
  geom_line(aes(x = ActPARCorr, y = AllmeanJVPSII_umol_eLs, linetype = as.factor(O2)), size = 0.3, show.legend = F) +
  geom_errorbar(aes(x = ActPARCorr, ymin = AllmeanJVPSII_umol_eLs - AllsdJVPSII_umol_eLs, ymax = AllmeanJVPSII_umol_eLs + AllsdJVPSII_umol_eLs, colour = as.factor(Strain)), width=0, size=0.3, show.legend = F) +
  geom_vline(xintercept = 90, linetype = "dotted", size = 0.4) +
  geom_text(data=data_textD, aes(x=750, y=0.75, label=label), size=6, parse = TRUE) +
  scale_fill_identity() +
  scale_colour_manual(values = c("palegreen3", "brown4"), name="", labels = lab2) +
  scale_shape_manual(values = c(16, 1), name="", labels = lab2) +
  scale_linetype_manual(values = c("solid", "dashed"), name="", labels = lab2) +
  ...
  scale_x_continuous(breaks=seq(0, 900, by = 300)) +
  coord_cartesian(ylim = c(0, 0.8), xlim = c(0, 900)) +
...
Stat$Strain <- factor(Stat$Strain)
# Three way Anova with interactions
model<-aov(JVPSII_umol_eLs~O2*ActPARCorr*Strain, data=Stat)
...
saveRDS(AnovaJVPSII, file.path(TableRdsPath, paste(Project, "Anova_JVPSII_eLsLRC.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
...
SolFitsExpStTurner <- ChlDataMeta %>%
  full_join(., SolFitsMetaLRCTrimChoosen, by = c("SampleID"="SampleID", "Run"="Run", "Strain"="Strain", "ExpDate"="ExpDate", "Par_ue"="Par_ue", "Photoperiod"="Photoperiod", 'O2'="O2", "WL"="WL", "PARPhotonDose_day"="PARPhotonDose_day")) 
...
GrowthRateSol<-SolFitsExpStTurnerMD %>%
  ...
  #   filter(WL != 730) %>%
  #Turner
  mutate(JVPSII_umol_eLs_Par_ue_ugChl = AllmeanJVPSII_umol_eLs/meanTurChl_ugL) %>% #umol e/ug ch/s
  #mutate(JVPSII_umol_eLs_HalfPar_ue_ugChl = AllmeanJVPSII_ETRqpOxbo_FoSigmax_m2psii/meanTurChl_ugL) %>% 
  
  mutate(JVPSII_umol_eLs_Par_ue_umolChl = AllmeanJVPSII_umol_eLs/meanTurChl_umolL) %>% #umol e/umol ch/s
  ...
  mutate(JVPSII_umol_eLs_Par_ue_cell = AllmeanJVPSII_umol_eLs/meanCellL_MDPico) 
...
# Cumulative uncalibrated JVPSII vs ActPAR
...
GrowthRateSoliExpDay %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = JVPSII_umol_eLs_Par_ue_umolChl_day, shape = as.factor(O2)), size = 3, show.legend = T) +
  geom_line(aes(x = ActPARCorr, y = JVPSII_umol_eLs_Par_ue_umolChl_day, linetype = as.factor(O2)), size = 0.3, show.legend = T) +
...
GrowthRateSol %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = JVPSII_umol_eLs_Par_ue_umolChl, shape = as.factor(O2)), size = 3, show.legend = T) +
  geom_line(aes(x = ActPARCorr, y = JVPSII_umol_eLs_Par_ue_umolChl, linetype = as.factor(O2)), size = 0.3, show.legend = T) +
  # geom_errorbar(aes(x = ActPARCorr, ymin = AllmeanJVPSII_ETRqpOxbo_FoSigmax_m2psii - AllsdJVPSII_ETRqpOxbo_FoSigmax_m2psii, ymax = AllmeanJVPSII_ETRqpOxbo_FoSigmax_m2psii + AllsdJVPSII_ETRqpOxbo_FoSigmax_m2psii, colour = as.factor(Strain)), width=0, size=0.3, show.legend = F) +
  geom_vline(xintercept = 90, linetype = "dotted", size = 0.7) +
  scale_fill_identity() +
  scale_colour_manual(values = c("palegreen3", "brown4"), name="", labels = lab2) +
...
MCGrowthSPAllFit21 <- GrowthRateSol %>%   
  ...
  mutate(GRCplatt_Model = map(data, ~nlsLM(JVPSII_umol_eLs_Par_ue_umolChl ~ grcplatt(I = ActPARCorr, a, b, Pmax),
...
MCGrowthSPAllFit0 <- GrowthRateSol %>%   
  filter(O2 == 0) %>% 
  nest(data = -c("Strain", "Ex_WL", "O2")) %>%  
  mutate(GRCplatt_Model = map(data, ~nlsLM(JVPSII_umol_eLs_Par_ue_umolChl ~ grcplatt(I = ActPARCorr, a, b, Pmax),
         data = .x,start = list(a = 0.1/90, b = 0.1/200, Pmax = 0.2),lower = c(0, 0, 0)))) %>%    
...
CalibJVPSIIpers<-GrowthRateSol %>%
  ...
  geom_point(aes(x = ActPARCorr, y = JVPSII_umol_eLs_Par_ue_umolChl), colour = "palegreen3", shape = 1, size = 3, show.legend = F, MultiCultiGrowth1PC21) +
  geom_point(aes(x = ActPARCorr, y = JVPSII_umol_eLs_Par_ue_umolChl), colour = "brown4", shape = 1, size = 3, show.legend = F, MultiCultiGrowth1PE21) +
  geom_point(aes(x = ActPARCorr, y = JVPSII_umol_eLs_Par_ue_umolChl), colour = "palegreen3", shape = 16, size = 3, show.legend = F, MultiCultiGrowth1PC0) +
  geom_point(aes(x = ActPARCorr, y = JVPSII_umol_eLs_Par_ue_umolChl), colour = "brown4", shape = 16, size = 3, show.legend = F, MultiCultiGrowth1PE0) +
...
Stat$Strain <- factor(Stat$Strain)
# Three way Anova with interactions
model<-aov(JVPSII_umol_eLs_Par_ue_umolChl~O2*ActPARCorr*Strain, data=Stat)
AnovaTest_jvpsii445<-data.frame(unclass(summary(model)), check.names = FALSE, stringsAsFactors = FALSE)
AnovaTest_jvpsii445$Ex_WL<-445
...
GrowthRateSol %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = ETRqpOxbo/JVPSII_umol_eLs_Par_ue_umolChl, shape = as.factor(O2)), size = 3, show.legend = T) +
  geom_vline(xintercept = 90, linetype = "dotted", size = 0.7) +
  scale_fill_identity() +
  scale_colour_manual(values = c("palegreen3", "brown4"), name="", labels = lab2) +
  scale_shape_manual(values = c(16, 1), name="", labels = lab2) +
...
GrowthRateSol %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanETRqpOxbo/JVPSII_umol_eLs_Par_ue_umolChl, shape = as.factor(O2)), size = 3, show.legend = T) +
  geom_vline(xintercept = 90, linetype = "dotted", size = 0.7) +
  scale_fill_identity() +
  scale_colour_manual(values = c("palegreen3", "brown4"), name="", labels = lab2) +
  scale_shape_manual(values = c(16, 1), name="", labels = lab2) +
