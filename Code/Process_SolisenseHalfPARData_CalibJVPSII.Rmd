...
GrowthRateSol<-SolFitsExpStTurnerMD %>%
  ...
  #   filter(WL != 730) %>%
  #Turner
  mutate(JVPSII_umol_eLs_Par_ue_ugChl = AllmeanJVPSII_ETRqpOxbo_FoSigmax_m2psii/meanTurChl_ugL) %>% #umol e/ug ch/s
  #mutate(JVPSII_umol_eLs_HalfPar_ue_ugChl = AllmeanJVPSII_ETRqpOxbo_FoSigmax_m2psii/meanTurChl_ugL) %>% 
  
  mutate(JVPSII_umol_eLs_Par_ue_umolChl = AllmeanJVPSII_ETRqpOxbo_FoSigmax_m2psii/meanTurChl_umolL) %>% #umol e/umol ch/s
  ...
  mutate(JVPSII_umol_eLs_Par_ue_cell = AllmeanJVPSII_ETRqpOxbo_FoSigmax_m2psii/meanCellL_MDPico) 
...
SolFitsMeta <- left_join(GrowthRateSoliExpDay, JVPSII_eLs_ModelTerms, by = join_by(Ex_WL)) |>
  select(-c(Slope_P)) |>
  mutate(JVPSII_eLs = JVPSII_umol_eLs_Par_ue_umolChl_day/Slope) |>
...
SolFitsMeta %>%
  ...
  # coord_cartesian(ylim = c(0, 10), xlim = c(0, 800)) +
  #labs(y=expression(paste(sigma,""["PSII"]*"' (nm"^"2"*" quanta"^"-1"*")")), x = "Actinic PAR ( µmol photons "~m^-2~""~s^-1~")") +
  labs(x = "Actinic PAR ( µmol photons "~m^-2~""~s^-1~")", y = ""~italic(JV)~""["PSII"]*" (µmol"~e^-~""~µmol~Chl~italic(a)~""^-1~""~d^-1~")") +
  ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(Strain), labeller = label_parsed) +
  theme_bw() 
```
