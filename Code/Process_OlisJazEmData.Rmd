...
Spectra<-OLISSpectraMetaPUR %>%
  ...
  # geom_area(aes(x = nm, y = PURNormBA77G, fill = "palegreen3"), alpha = 0.5, show.legend = F) +
  geom_line(aes(x = nm, y = Absorbance, colour = as.factor(Strain), linetype = as.factor(Strain)), show.legend = T, size = 0.4) +
...
Spectra<-OLISSpectraMetaPUR %>%
  ...
  coord_cartesian(ylim = c (-0.01, 1.6)) +
  ggh4x::facet_nested(rows = vars(WL),  cols = vars(factor(Oxygen, levels=c("250~µM","2.5~µM"))), labeller = label_parsed) +
...
Spectra<-OLISSpectraMetaPUR %>%
  ...
  # geom_area(aes(x = nm, y = PURNormBA77G, fill = "palegreen3"), alpha = 0.5, show.legend = F) +
  geom_line(aes(x = nm, y = AbsNorm440, linetype = as.factor(O2)), show.legend = F, size = 0.4) +
...
Spectra<-OLISSpectraMetaPUR %>%
  ...
  # geom_area(aes(x = nm, y = PURNormBA77G, fill = "palegreen3"), alpha = 0.5, show.legend = F) +
  geom_line(aes(x = nm, y = Absorbance, colour = as.factor(Strain), linetype = as.factor(O2)), show.legend = T, size = 0.6) +
  geom_line(aes(x = nm, y = Absorbance, linetype = as.factor(O2)), colour = "brown4", show.legend = F, size = 0.6, OLISSpectraMetaPUR127405) +
  scale_colour_manual(values = c("palegreen3", "brown4", "brown4"), name="", labels = lab1) +
  scale_linetype_manual(values = c("solid", "dashed", "dashed"), name="", labels = lab2) +
...
```{r}
MCDataHourSVR <- OLISAllPURTidyChoosen %>%  
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") %>% 
...
MultiCultiGrowth1PC21<-OLISAllPURTidyChoosen %>% 
    mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") %>%
  filter(Strain == "PC-rich") %>% 
  filter(O2 == 21)
MultiCultiGrowth1PE21<-OLISAllPURTidyChoosen %>% 
    mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") %>%
  filter(Strain == "PE-rich") %>% 
    filter(O2 == 21)
  
MultiCultiGrowth1PC0<-OLISAllPURTidyChoosen %>% 
    mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") %>%
  filter(Strain == "PC-rich") %>% 
  filter(O2 == 0)
MultiCultiGrowth1PE0<-OLISAllPURTidyChoosen %>% 
    mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") %>%
  filter(Strain == "PE-rich") %>% 
    filter(O2 == 0)
