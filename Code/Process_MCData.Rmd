...
```{r read ProcessFile}
ProcessFile <- "../Data/ImportedData/ImportedMCData/20220407_PICO_MCMIX006_RUN61_TargetDataMetaFilter.Rds"
...
ProcessDataNestGrowth <- ProcessDataNestGrowth %>%
  mutate(OD720_logistic = map(tubedata, possibly(~nlsLM(OD720 ~ logistic_eqn(x = time, Lmax, Lmu, Lintercept),
                            ...
                            upper = c(Lmax = max(.$rollmeanOD720, na.rm = TRUE) * 1.5,
                                      Lmu = (log(max(.$rollmeanOD720, na.rm = TRUE)) - log(min(.$rollmeanOD720, na.rm = TRUE)))/max(.$time),
                                      Lintercept = max(.$rollmeanOD720, na.rm = TRUE))), 
                            otherwise = NULL)),
  ...
  mutate(deltaOD_TimetoMax = as.numeric(ifelse(deltaOD_TimetoMax == "NULL" , NA, deltaOD_TimetoMax))
   ...
   mutate(deltaODtrunc_logistic = map(tubedata, possibly(~nlsLM(deltaOD ~ logistic_eqn(x = time, Lmax, Lmu, Lintercept),
                            ...
                            upper = c(Lmax = max(.$rollmeandeltaOD, na.rm = TRUE) * 1.5,
                                      Lmu = (log(max(.$rollmeandeltaOD, na.rm = TRUE)) - log(min(.$rollmeandeltaOD, na.rm = TRUE)))/max(.$time),
                                      Lintercept = max(.$rollmeandeltaOD, na.rm = TRUE))), 
                            otherwise = NULL)),
