...
```{r read ProcessFile}
ProcessFile <- "../Data/ImportedData/ImportedMCData/20230830_PICO_MCMIX006_RUN124_TargetDataMetaFilter.Rds"
...
ProcessData <- readRDS(ProcessFile)  %>%
  ungroup()  %>% 
    filter(time <= 192)  # after 8 day - stationary phase - 
    #filter(time > 72) # weird problem with fits - 
...
ProcessDataNestGrowth <- ProcessDataNestGrowth %>%
  mutate(OD720_logistic = map(tubedata, possibly(~nlsLM(OD720+0.05 ~ logistic_eqn(x = time, Lmax, Lmu, Lintercept),
                            data = .x,
                            start = list(Lmax = max(.$rollmeanOD720+0.05, na.rm = TRUE),  Lmu = (log(max(.$rollmeanOD720+0.05, na.rm = TRUE)) - log(min(.$rollmeanOD720+0.05, na.rm = TRUE)))/max(.$time), Lintercept = min(.$rollmeanOD720+0.05, na.rm = TRUE)),
                            control = list(maxiter = 500),
                            lower = c(Lmax = 0,
                                      Lmu = 0,
                                      Lintercept = 0),
                            upper = c(Lmax = max(.$rollmeanOD720+0.05, na.rm = TRUE) * 1.5,
                                      Lmu = (log(max(.$rollmeanOD720+0.05, na.rm = TRUE)) - log(min(.$rollmeanOD720+0.05, na.rm = TRUE)))/max(.$time),
                                      Lintercept = max(.$rollmeanOD720+0.05, na.rm = TRUE))), 
                            otherwise = NULL)),
  ...
  mutate(OD720trunc_logistic = map(tubedata, possibly(~nlsLM(OD720+0.05 ~ logistic_eqn(x = time, Lmax, Lmu, Lintercept),
                            data = .x %>%
                            filter(OD720+0.05 <= (max(OD720+0.05) * TruncFactor)),
                            start = list(Lmax = max(.$rollmeanOD720+0.05, na.rm = TRUE),  Lmu = (log(max(.$rollmeanOD720+0.05, na.rm = TRUE)) - log(min(.$rollmeanOD720+0.05, na.rm = TRUE)))/max(.$time), Lintercept = min(.$rollmeanOD720+0.05, na.rm = TRUE)),
                            control = list(maxiter = 500),
                            lower = c(Lmax = 0,
                                      Lmu = 0,
  ...
  mutate(OD720trunc_TimetoMax = as.numeric(ifelse(OD720trunc_TimetoMax == "NULL" , NA, OD720trunc_TimetoMax))
   ...
   mutate(deltaOD_logistic = map(tubedata, possibly(~nlsLM(deltaOD+0.05 ~ logistic_eqn(x = time, Lmax, Lmu, Lintercept),
                            data = .x,
                            start = list(Lmax = max(.$rollmeandeltaOD+0.05, na.rm = TRUE),  Lmu = (log(max(.$rollmeandeltaOD+0.05, na.rm = TRUE)) - log(min(.$rollmeandeltaOD+0.05, na.rm =  TRUE)))/max(.$time), Lintercept = min(.$rollmeandeltaOD+0.05, na.rm = TRUE)),
                            control = list(maxiter = 500),
                            lower = c(Lmax = 0,
                                      Lmu = 0,
                                      Lintercept = 0),
                            upper = c(Lmax = max(.$rollmeandeltaOD+0.05, na.rm = TRUE) * 1.5,
                                      #Lmu = (log(max(.$rollmeandeltaOD, na.rm = TRUE)) - log(0.001))/max(.$time),
                                      Lmu = (log(max(.$rollmeandeltaOD+0.05, na.rm = TRUE)) - log(min(.$rollmeandeltaOD+0.05, na.rm = TRUE)))/max(.$time),
                                      Lintercept = max(.$rollmeandeltaOD+0.05, na.rm = TRUE))), 
                            otherwise = NULL)),
...
```{r}
#Test35 <- ProcessDataNestGrowth %>% filter(SampleID =="SySl1335")
```
