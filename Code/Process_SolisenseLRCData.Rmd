...
# Merge Turner with catalog to check missing data

```{r combine Solisense and MetaData catalog, warning = FALSE, echo=FALSE}
ChlDataCatalog <- CultureCatalog %>%
  left_join(., ChlData, by = c("SampleID" = "SampleID"))

ChlDataCatalog <- ChlDataCatalog %>%
group_by(SampleID) %>%
  arrange(ObsDate) %>%
...
ChlDataCatalog %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = TurChl_ugL, colour = as.factor(Strain)), size = 3.5, show.legend = T) +
  scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
...
--------------
### Problem with E_days - chack this Sylwia!

```{r}
#colnames(SolFitsMetaLRCGrowth)

SolFitsMetaLRCGrowth<-SolFitsMetaLRCGrowth %>% 
  select(-c(data, lrcplatt_Model, estimate_a, estimate_b, estimate_Pmax, std.error_a, std.error_b, std.error_Pmax, E_days))
```


# Merge Solisense with chlorophyll Turner data 

```{r}
SolFitsExpStTurner <- ChlDataCatalog %>%
  full_join(., SolFitsMetaLRCGrowth, by = c("SampleID"="SampleID", "Strain"="Strain", "ObsDate"="ObsDate", "Run"="Run", "Par_ue"="Par_ue", "ExpDate"="ExpDate", "ExpEndDate"="ExpEndDate","Photoperiod"="Photoperiod", "MC"="MC", "Tube"="Tube", "WL"="WL", "O2"="O2", "LightShape"="LightShape", "PARPhotonDose_day"="PARPhotonDose_day")) 
```
...
SolFitsExpStTurner %>% 
  ggplot() +
  geom_point(aes(x = Ex_WL, y = TurChl_ugL, colour = as.factor(Strain)), size = 3.5, show.legend = T) +
  scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
    ggh4x::facet_nested(cols = vars(WL), rows = vars(factor(O2, levels=c("21","0"))), labeller = label_parsed) +
  #ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(Strain), labeller = label_parsed) +
  theme_bw()
...
SolFitsExpStTurner %>% 
  ggplot() +
  geom_point(aes(x = TurChl_ugL, y = JVPSII_umol_eLs_Par_ue, colour = as.factor(Strain)), size = 3.5, show.legend = T) +
  scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
    ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(factor(O2, levels=c("21","0"))), labeller = label_parsed) +
  #ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(Strain), labeller = label_parsed) +
  theme_bw()
...
```{r}
SolFitsTurnerMDPico<-SolFitsTurnerMDPico %>% 
...
```{r save rds}
saveRDS(SolFitsTurnerMDPico, file.path(DataOut, paste(Project, "Processed_SolisenseJVPSII.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
