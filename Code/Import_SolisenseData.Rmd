...
zip_file <- file.path("..", "Data", "RawData", "SolisenseOSDataSS.zip")
...
SolFitsTrimOS <- SolFitsOS %>%
  ...
  mutate(Filename = str_remove(string = Filename, pattern = "/RawData/SolisenseOSDataSS.zip/"),
...
```{r save rds}
# saveRDS(SolFitsTrimOS_lightsteps, file.path(DataOut, paste(Project, "Imported_SolFitsTrimOS_lightstepsAll.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
...
SolFitsTrimMetaOS_param <- SolFitsTrimMetaOS %>%
  group_by(SourceDataFile, Filename, SampleID, ObsDate, Ex_WL, TempCont, O2) %>%
...
```{r save rds}
# saveRDS(SolFitsTrimMetaDarkafterLightOS, file.path(DataOut, paste(Project, "Imported_SolFitsTrimMetaDarkafterLightOSAll.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
...
SolFitsMeta %>%
  ...
  facet_grid(rows = vars(Ex_WL), cols = vars(WL, O2)) +
  theme_bw()
# 
...
```{r save plot}
ggsave(file = file.path(PlotsPath, paste("Solisense2_SupPlot",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
...
```{r save rds}
saveRDS(SolFitsMeta, file.path(DataOut, paste(Project, "Imported_SolisenseLightnew.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)

saveRDS(SolFitsMetaDarkafterLight, file.path(DataOut, paste(Project, "Imported_SolisenseDarkafterLightnew.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
