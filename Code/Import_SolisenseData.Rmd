...
zip_file <- file.path("..", "Data", "RawData", "SolisenseNSDatas.zip")
...
SolFitsTrimNS <- SolFitsNS %>%
  ...
  mutate(Filename = str_remove(string = Filename, pattern = "/RawData/SolisenseNSDatas.zip/"),
         Filename = str_remove(string = Filename, pattern = "_fit.csv")
         ) %>%
...
SolFitsMeta %>%
  ...
  facet_grid(rows = vars(Ex_WL), cols = vars(Photoperiod, WL, O2)) +
  theme_bw()
# 
