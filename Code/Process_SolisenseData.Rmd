...
#Filter outliers
```{r}
SolFitsMeta1 <-SolFitsMeta %>%
  filter(Strain == "PC-rich") %>% 
  filter(O2 == 21) %>% 
  filter(Ex_WL== 470 | Ex_WL == 505) %>% 
  filter(Sig > 400)
SolFitsMeta2 <-SolFitsMeta %>%
  filter(Strain == "PE-rich") 
SolFitsMeta3 <-SolFitsMeta %>%
  filter(Strain == "PC-rich") %>% 
  filter(O2 == 0)  
SolFitsMeta4 <-SolFitsMeta %>%
  filter(Strain == "PC-rich") %>% 
  filter(O2 == 21) %>%     
  filter(Ex_WL!= 470 | Ex_WL != 505) 
SolFitsMeta5 <-SolFitsMeta %>%
  filter(Strain == "PC-rich") %>% 
  filter(O2 == 21) %>% 
  filter(Ex_WL== 470 | Ex_WL == 505) %>% 
  filter(Sig < 400)
...
ss<-rbind(SolFitsMeta2, SolFitsMeta3, SolFitsMeta4)
...
SolFitsMeta %>%
  ...
  filter(O2 == 0) %>% 
  filter(WL != "WW") %>%
  #filter(ObsDate == "2022-06-16") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = Sig), size = 3.5, show.legend = T) +
  #scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
  scale_fill_identity() +
  ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(Strain, O2, WL), labeller = label_parsed) +
  theme_bw()
...
SolFitsMeta %>%
  ...
  #filter(ObsDate == "2022-06-16") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = Fm), size = 3.5, show.legend = T) +
  #scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
  scale_fill_identity() +
  ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(Strain, O2, WL), labeller = label_parsed) +
  theme_bw()
...
SolFitsMeta %>%
  ...
  #filter(ObsDate == "2022-06-16") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = FvFm), size = 3.5, show.legend = T) +
  #scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
  scale_fill_identity() +
  ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(Strain, O2, WL), labeller = label_parsed) +
  theme_bw()
