...
SolFitsMetaLRC %>%
  ...
  filter(E_days>=4) %>% 
  ggplot() +
  geom_point(aes(x = WLNum, y = TauAv, colour = as.factor(Strain)), size = 3.5, show.legend = T) +
  scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
  stat_wl_strip(aes(x = WLNum), ymin = -Inf, ymax = -0.025, alpha = 0.5) +
  scale_fill_identity() +
  ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(factor(O2, levels=c("21","0"))), labeller = label_parsed) +
...
SolFitsMetaLRC <- SolFitsMetaLRC %>%
  ...
  summarize(SampleID, Ex_WL, FilenameSolisense, Run, Strain, ExpDate, Par_ue, Photoperiod, O2, WL, WLNum, Oxygen, PARPhotonDose_day, ObsDate, ObsTime, ObsDateTime, Sig, Sig_nm2psii, ActPARCorr, ActPARCorr_photonsm2s, Sigdark_nm2psii, FoOxbo, qpOxbo, JVPSII_ETRqpOxbo_FoSigmax_m2psii, E_days,
...
SolFitsMetaLRC <- SolFitsMetaLRC %>%
  ...
  summarize(SampleID, Ex_WL, FilenameSolisense, Run, Strain, ExpDate, Par_ue, Photoperiod, O2, WL, WLNum, Oxygen, PARPhotonDose_day, ObsDate, ObsTime, ObsDateTime, Sig, Sig_nm2psii, meanSig_nm2psii, sdSig_nm2psii, meanSigdark_nm2psii, sdSigdark_nm2psii, ActPARCorr, ActPARCorr_photonsm2s, Sigdark_nm2psii, FoOxbo, qpOxbo, JVPSII_ETRqpOxbo_FoSigmax_m2psii, E_days, meanTauAv, sdTauAv, meanqpOxbo, sdqpOxbo, 
...
SolFitsMetaLRC %>%
  ...
  filter(E_days>=4) %>% 
  ggplot() +
  geom_point(aes(x = WLNum, y = meanSig_nm2psii, colour = as.factor(Strain)), size = 3.5, show.legend = T) +
  scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
  stat_wl_strip(aes(x = WLNum), ymin = -Inf, ymax = -0.025, alpha = 0.5) +
  scale_fill_identity() +
  ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(factor(O2, levels=c("21","0"))), labeller = label_parsed) +
...
SolFitsMetaLRC %>%
  ...
  filter(E_days>=4) %>% 
  ggplot() +
  geom_point(aes(x = WLNum, y = AllmeanTauAv, colour = as.factor(O2)), size = 3.5, show.legend = T) +
  #scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
  stat_wl_strip(aes(x = WLNum), ymin = -Inf, ymax = -0.025, alpha = 0.5) +
  scale_fill_identity() +
...
drectblue <- data.frame(Ex_WL = c("445", "445"), Strain = c("PC-rich", "PE-rich"), WL = c("450", "450"), x1=c(-35), x2=c(835), y1=c(-0.5), y2=c(10.5))

drectgreen <- data.frame(Ex_WL = c("535", "535"), Strain = c("PC-rich", "PE-rich"), WL = c("530", "530"), x1=c(-35), x2=c(835), y1=c(-0.5), y2=c(10.5))

drectred <- data.frame(Ex_WL = c("590", "590"), Strain = c("PC-rich", "PE-rich"), WL = c("620", "620"), x1=c(-35), x2=c(835), y1=c(-0.5), y2=c(10.5))
...
SolFitsMetaLRC %>%
  ...
  ggplot() +
    ...
    geom_rect(data=drectblue, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), fill = "transparent", color="blue", size=1, alpha=0.5) +
    geom_rect(data=drectgreen, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), fill = "transparent", color="darkgreen", size=1, alpha=0.5) +
    geom_rect(data=drectred, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), fill = "transparent", color="red", size=1, alpha=0.5) +
  
  geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii, colour = as.factor(Strain)), size = 2, show.legend = F) +
  ...
  geom_line(aes(x = ActPARCorr, y = AllmeanSig_nm2psii, linetype = as.factor(O2)), size = 0.6, show.legend = T) +
  ...
  labs(y=expression(paste(sigma,""["PSII"]*"' (nm"^"2"*" quanta"^"-1"*")")), x = "ActPAR") +
  ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(Strain, WL), labeller = label_parsed) +
  theme_bw() 
...
```{r}
MCDataHourSVR <- SolFitsMetaLRC %>% 
  select(c(Strain, O2, Oxygen, WLNum, Ex_WL, WL, ActPARCorr, AllmeanSig_nm2psii)) %>% 
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") %>% 
  filter(WL == 450 | WL == 530 |  WL == 620) %>%
  filter(Ex_WL == 445 | Ex_WL == 535 | Ex_WL == 590) 
...
train_and_predict_svm <- function(data) {
  svr_model <- svm(AllmeanSig_nm2psii ~ ActPARCorr, data = data, kernel = 'radial',
                   cost = 5, gamma = 10)
...
MCDataHourSVR <- MCDataHourSVR %>%
  group_by(Strain, O2, Oxygen, WLNum, WL, Ex_WL) %>%
  nest() %>%
...
MCDataHourSVR %>%
  ...
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii), alpha = 0.4) +
  geom_line(aes(x = ActPARCorr, y = svr_pred, color = 'SVR_radial')) +
  geom_line(aes(x = ActPARCorr, y = AllmeanSig_nm2psii)) +
...
SolFitsMetaLRC %>%
  ...
  filter(Ex_WL == 445 | Ex_WL == 535 | Ex_WL == 590) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii, colour = as.factor(Strain)), size = 4, show.legend = F) +
  geom_point(aes(x = ActPARCorr, y = Sig_nm2psii, colour = as.factor(Strain)), size = 1, alpha = 0.2, show.legend = F) +
  geom_errorbar(aes(x = ActPARCorr, ymin = AllmeanSig_nm2psii - AllsdSig_nm2psii, ymax = AllmeanSig_nm2psii + AllsdSig_nm2psii, colour = as.factor(Strain)), width=0, size=0.3, show.legend = F) +
...
MultiCultiGrowth1PC21<-SolFitsMetaLRC %>% 
    mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL == 450 | WL == 530 |  WL == 620) %>%
  filter(Ex_WL == 445 | Ex_WL == 535 | Ex_WL == 590) %>%
  filter(Strain == "PC-rich") %>% 
  filter(O2 == 21)
MultiCultiGrowth1PE21<-SolFitsMetaLRC %>% 
    mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL == 450 | WL == 530 |  WL == 620) %>%
  filter(Ex_WL == 445 | Ex_WL == 535 | Ex_WL == 590) %>%
...
MultiCultiGrowth1PC0<-SolFitsMetaLRC %>% 
    mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL == 450 | WL == 530 |  WL == 620) %>%
  filter(Ex_WL == 445 | Ex_WL == 535 | Ex_WL == 590) %>%
  filter(Strain == "PC-rich") %>% 
  filter(O2 == 0)
MultiCultiGrowth1PE0<-SolFitsMetaLRC %>% 
    mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL == 450 | WL == 530 |  WL == 620) %>%
  filter(Ex_WL == 445 | Ex_WL == 535 | Ex_WL == 590) %>%
  filter(Strain == "PE-rich") %>% 
...
SolFitsMetaLRC %>%
  ...
  filter(Ex_WL == 445 | Ex_WL == 535 | Ex_WL == 590) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii, shape = as.factor(O2)), size = 4, show.legend = T) +
  #geom_point(aes(x = ActPARCorr, y = Sig_nm2psii, colour = as.factor(Strain)), size = 1, alpha = 0.2, show.legend = F) +
  
    geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii), colour = "palegreen3", shape = 1, size = 4, show.legend = F, MultiCultiGrowth1PC21) +
    geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii), colour = "brown4", shape = 1, size = 4, show.legend = F, MultiCultiGrowth1PE21) +
    geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii), colour = "palegreen3", shape = 16, size = 4, show.legend = F, MultiCultiGrowth1PC0) +
    geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii), colour = "brown4", shape = 16, size = 4, show.legend = F, MultiCultiGrowth1PE0) +
  ...
  geom_vline(xintercept = 180, linetype = "dotted", size = 0.7) +
  
  geom_rect(data=drectblue, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), color="steelblue3", fill="steelblue3", alpha = 0.2, size=1) +
  geom_rect(data=drectgreen, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), color="limegreen", fill="limegreen", alpha = 0.2, size=1) +
  geom_rect(data=drectred, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), color="coral", fill="coral", alpha = 0.2, size=1) +
...
```{r save plot}
#ggsave(file = file.path(FigPath, paste("Fig_SigLRC",".png",sep = "")), height=6, width= 6,  dpi = 300, limitsize = TRUE)
```
...
```{r save plot}
#ggsave(file = file.path(FigPath, paste("Fig_SigLRCTrim",".png",sep = "")), height=6, width= 6,  dpi = 300, limitsize = TRUE)
```
...
```{r}
MCDataHourSVR <- SolFitsMetaLightCorr %>% 
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") %>% 
    filter(WL != 405) %>%
    filter(WL != 660) %>%
...
# Function to train an SVM model and make predictions
train_and_predict_svm <- function(data) {
  svr_model <- svm(meanSig_nm2psii ~ WLNum, data = data, kernel = 'radial',
                   cost = 5, gamma = 10)
  data$svr_pred <- predict(svr_model, data)
  return(data)
}
...
RMSE_by_Tube <- MCDataHourSVR %>%
  group_by(Strain, O2, Oxygen) %>%
  summarise(RMSE = sqrt(mean((meanSig_nm2psii - svr_pred)^2)))
...
MCDataHourSVR %>%
  ...
  ggplot() +
  geom_point(aes(x = WLNum, y = meanSig_nm2psii), alpha = 0.4) +
  geom_line(aes(x = WLNum, y = svr_pred, color = 'SVR_radial')) +
  geom_line(aes(x = WLNum, y = meanSig_nm2psii)) +
...
```{r save plot}
#ggsave(file = file.path(FigPath, paste("Fig_SigCorr",".png",sep = "")), height=6, width= 6,  dpi = 300, limitsize = TRUE)
```
...
```{r save rds}
saveRDS(SolFitsExpStTurnerMD, file.path(DataOut, paste(Project, "Processed_SolisenseDataCorr.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
...
```{r save rds}
saveRDS(SolFitsExpStTurnerMD445590, file.path(DataOut, paste(Project, "Processed_SolisenseData445590.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
