...
OLISSpectraFile <- "../Data/ProcessedData/ProcessedOlisJazData/BalticO2_Processed_OlisSpectraTidy.Rds"
...
MCGrowthRate %>%
  ...
  ggh4x::facet_nested(rows = vars(Strain, O2), cols = vars(WL)) +
  theme_bw()
...
model<-aov(deltaOD_Lmu_corr~Strain*O2*WL, data=MCGrowthRateStats)
AnovaTest_GrowthRate<-data.frame(unclass(summary(model)), check.names = FALSE, stringsAsFactors = FALSE)
TukeyHSDTest_GrowthRate<-TukeyHSD(model, which = c("Strain", "O2", "WL"))
```
