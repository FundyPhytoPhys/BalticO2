...
JazSpecTrim %>%
  ggplot() +
  geom_point(aes(x = Wavelength, y = rollmeanCounts, colour = as.factor(Strain)), alpha = 0.9, size = 3, show.legend = T) +
  ggh4x::facet_nested(cols = vars(Par_ue), rows = vars(WL), labeller = label_parsed) +
  theme_bw() 
```
