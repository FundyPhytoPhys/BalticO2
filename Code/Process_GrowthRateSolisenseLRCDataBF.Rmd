...
SolFitsAllPigmentfitPhycoChlalinear %>% 
  ...
  ggplot() +
  geom_point(aes(y = deltaOD_Lmu_corr, x = meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day, colour=as.factor(Strain)), size = 3.5, show.legend = T, MCGrowthJVPSIISPmeanDummy) + 
  geom_point(aes(y = deltaOD_Lmu_corr, x = JVPSII_umol_eLs_HalfPar_ue_umolChl_day, colour=as.factor(Strain)), size = 2, show.legend = T, alpha = 0.3, MCGrowthJVPSIISPmeanDummy) + 
  
  geom_line(aes(x = JVPSII_umol_eLs_HalfPar_ue_umolChl_day, y = `.fitted`), show.legend = F) +  
...
```{r}
# Strain across O2
Stats77_ExpSt<-MCGrowthJVPSIISPmeanDummy %>%
  filter(Strain == "PC-rich") 
  fit <- lm(deltaOD_Lmu_corr ~ JVPSII_umol_eLs_HalfPar_ue_umolChl_day + O2 + JVPSII_umol_eLs_HalfPar_ue_umolChl_day:O2,data=Stats77_ExpSt)
...
Stats127_ExpSt<-MCGrowthJVPSIISPmeanDummy %>%
  filter(Strain == "PE-rich") 
  fit <- lm(deltaOD_Lmu_corr ~ JVPSII_umol_eLs_HalfPar_ue_umolChl_day + O2 + JVPSII_umol_eLs_HalfPar_ue_umolChl_day:O2,data=Stats127_ExpSt)
x <- cbind(t(as.numeric(coefficients(fit))), t(as.numeric(summary(fit)$coefficients[4, 4])), t(as.numeric(summary(fit)$coefficients[4, 3])), t(as.numeric(summary(fit)$coefficients[4, 2])))
...
Stats77_ExpSt<-MCGrowthJVPSIISPmeanDummy %>%
  filter(O2 == 0) 
  fit <- lm(deltaOD_Lmu_corr ~ JVPSII_umol_eLs_HalfPar_ue_umolChl_day + Strain + JVPSII_umol_eLs_HalfPar_ue_umolChl_day:Strain,data=Stats77_ExpSt)
x <- cbind(t(as.numeric(coefficients(fit))), t(as.numeric(summary(fit)$coefficients[4, 4])), t(as.numeric(summary(fit)$coefficients[4, 3])), t(as.numeric(summary(fit)$coefficients[4, 2])))
...
Stats127_ExpSt<-MCGrowthJVPSIISPmeanDummy %>%
  filter(O2 == 21) 
  fit <- lm(deltaOD_Lmu_corr ~ JVPSII_umol_eLs_HalfPar_ue_umolChl_day + Strain + JVPSII_umol_eLs_HalfPar_ue_umolChl_day:Strain,data=Stats127_ExpSt)
x <- cbind(t(as.numeric(coefficients(fit))), t(as.numeric(summary(fit)$coefficients[4, 4])), t(as.numeric(summary(fit)$coefficients[4, 3])), t(as.numeric(summary(fit)$coefficients[4, 2])))
...
data_text_PC1<- data.frame(Strain  = c("PC-rich"), label = c('a'))
data_text_PC2<- data.frame(Strain  = c("PC-rich"), label = c('b'))
data_text_PE1<- data.frame(Strain  = c("PE-rich"), label = c('c'))
data_text_PE2<- data.frame(Strain  = c("PE-rich"), label = c('d'))

data_text_Anova<- data.frame(Strain  = c("PE-rich"),  label = c('  , \u03B1 = 0.05'))
data_text_Anova2<- data.frame(Strain  = c("PE-rich"),  label = c('italic(t)~-test'))


MultiCultiGrowth1PC21<-MCGrowthJVPSIISPmeanDummy %>% 
    mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") %>%
  filter(Strain == "PC-rich") %>% 
  filter(O2 == 21)
MultiCultiGrowth1PE21<-MCGrowthJVPSIISPmeanDummy %>% 
    mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") %>%
  filter(Strain == "PE-rich") %>% 
    filter(O2 == 21)
  
MultiCultiGrowth1PC0<-MCGrowthJVPSIISPmeanDummy %>% 
    mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") %>%
  filter(Strain == "PC-rich") %>% 
  filter(O2 == 0)
MultiCultiGrowth1PE0<-MCGrowthJVPSIISPmeanDummy %>% 
    mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") %>%
  filter(Strain == "PE-rich") %>% 
    filter(O2 == 0)
...
SolFitsAllPigmentfitPhycoChlalinear %>% 
  unnest(Linear_Model_Predict) %>% 
  ggplot() +
  geom_point(aes(meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day, deltaOD_Lmu_corr, shape = as.factor(O2), shape = as.factor(O2)), alpha = 0.8, size = 4, show.legend = T, MCGrowthJVPSIISPmeanDummy) +
  geom_point(aes(y = deltaOD_Lmu_corr, x = JVPSII_umol_eLs_HalfPar_ue_umolChl_day, colour = as.factor(Strain), shape=as.factor(O2)), size = 2, show.legend = F, alpha = 0.3, MCGrowthJVPSIISPmeanDummy) + 
  ...
  geom_line(aes(x = JVPSII_umol_eLs_HalfPar_ue_umolChl_day, y = `.fitted`), show.legend = F, colour = "blue", size = 0.7, PC21, linetype = "dotted") +  
  geom_line(aes(x = JVPSII_umol_eLs_HalfPar_ue_umolChl_day, y = `.fitted`), show.legend = F, colour = "blue", size = 0.7, PC0, linetype = "solid") +  
  geom_line(aes(x = JVPSII_umol_eLs_HalfPar_ue_umolChl_day, y = `.fitted`), show.legend = F, colour = "blue", size = 0.7, PE21, linetype = "dotted") +  
  geom_line(aes(x = JVPSII_umol_eLs_HalfPar_ue_umolChl_day, y = `.fitted`), show.legend = F, colour = "blue", size = 0.7, PE0, linetype = "solid") +  
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_GrowthRateJVPSII_2",".png",sep = "")), height=5, width= 6,  dpi = 300, limitsize = TRUE)
```
