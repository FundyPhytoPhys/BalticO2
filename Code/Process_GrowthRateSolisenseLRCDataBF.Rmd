...
MCGrowthJVPSIISPmean %>% 
  filter(WL != "WW") %>% 
ggplot() +
  geom_point(aes(meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day, deltaOD_Lmu_corr, colour = as.factor(Ex_WL), shape = as.factor(O2)), alpha = 0.8, size = 3.5, show.legend = T) +
    geom_smooth(aes(meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day, deltaOD_Lmu_corr, colour = as.factor(O2)), alpha = 0.2, size = 1, method = "lm") +
