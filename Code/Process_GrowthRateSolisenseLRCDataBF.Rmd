...
SolFitsAllPigmentfitPhycoChlalinear <- MCGrowthJVPSIISPmean %>%   
  ...
  select(c(Strain, Par_ue, Photoperiod, deltaOD_Lmu_corr, meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day, O2, WL)) %>%  
  nest(data = -c("O2", "Strain")) %>%   
  mutate(Linear_Model = map(data, ~lm(deltaOD_Lmu_corr ~ meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day, data = .x))) %>%     
...
Stats77_ExpSt<-MCGrowthJVPSIISPmean %>%
  filter(Strain == "PC-rich") 
  fit <- lm(deltaOD_Lmu_corr ~ meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day + O2 + meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day:O2,data=Stats77_ExpSt)
...
Stats127_ExpSt<-MCGrowthJVPSIISPmean %>%
  filter(Strain == "PE-rich") 
  fit <- lm(deltaOD_Lmu_corr ~ meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day + O2 + meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day:O2,data=Stats127_ExpSt)
x <- cbind(t(as.numeric(coefficients(fit))), t(as.numeric(summary(fit)$coefficients[4, 4])), t(as.numeric(summary(fit)$coefficients[4, 3])), t(as.numeric(summary(fit)$coefficients[4, 2])))
...
Stats77_ExpSt<-MCGrowthJVPSIISPmean %>%
  filter(O2 == 0) 
  fit <- lm(deltaOD_Lmu_corr ~ meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day + Strain + meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day:Strain,data=Stats77_ExpSt)
x <- cbind(t(as.numeric(coefficients(fit))), t(as.numeric(summary(fit)$coefficients[4, 4])), t(as.numeric(summary(fit)$coefficients[4, 3])), t(as.numeric(summary(fit)$coefficients[4, 2])))
...
Stats127_ExpSt<-MCGrowthJVPSIISPmean %>%
  filter(O2 == 21) 
  fit <- lm(deltaOD_Lmu_corr ~ meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day + Strain + meanJVPSII_umol_eLs_HalfPar_ue_umolChl_day:Strain,data=Stats127_ExpSt)
x <- cbind(t(as.numeric(coefficients(fit))), t(as.numeric(summary(fit)$coefficients[4, 4])), t(as.numeric(summary(fit)$coefficients[4, 3])), t(as.numeric(summary(fit)$coefficients[4, 2])))
...
```{r}
saveRDS(Anova_GrowthJVPSII, file.path(TableRdsPath, paste(Project, "Anova_GrowthJVPSII_Model.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
...
data_text_PC1<- data.frame(Strain  = c("PC-rich"), label = c('a'))
data_text_PC2<- data.frame(Strain  = c("PC-rich"), label = c('a'))
data_text_PE1<- data.frame(Strain  = c("PE-rich"), label = c('b'))
data_text_PE2<- data.frame(Strain  = c("PE-rich"), label = c('c'))
...
FigGrowthPig<-MCGrowthPig %>%
  ...
  geom_smooth(aes(x = meanPhycoChlaRatio, y = deltaOD_Lmu_corr, linetype = as.factor(O2)), size = 0.7, colour = "blue", show.legend = F, alpha = 0, method = "lm") +
  ...
  geom_text(data=data_text_PC2, aes(x=3.9, y=0.16, label=label), colour = "blue", size=5) +
