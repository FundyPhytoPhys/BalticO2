...
GrowthRateSol %>%
  filter(WL!="WW") %>% 
  ggplot() +
  geom_point(aes(x = JVPSII_umol_eLs_Par_ue, y = deltaOD_Lmu_corr, colour = as.factor(Strain)),  alpha = 0.8, size = 3, show.legend = T) +
  scale_colour_manual(values = c("palegreen3", "brown4"), name="", labels = lab1) +
  scale_shape_manual(values = c(16, 18), name="", labels = lab1) +
  scale_linetype_manual(values = c("solid", "dashed"), name="", labels = lab1) +
  ggh4x::facet_nested(cols = vars(WL),rows = vars(factor(Oxygen, levels=c("250~µM","2.5~µM"))), labeller = label_parsed) +
...
ggplot() +
  ...
  scale_shape_manual(values = c(16, 18), name="", labels = lab1) +
  scale_linetype_manual(values = c("solid", "dashed"), name="", labels = lab1) +
  ggh4x::facet_nested(cols = vars(WL),rows = vars(Ex_WL, factor(Oxygen, levels=c("250~µM","2.5~µM"))), labeller = label_parsed) +
  theme_bw() 
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_GrowthRate_JVPSII_Chla2",".png",sep = "")), height=4, width= 6,  dpi = 300, limitsize = TRUE)
```
